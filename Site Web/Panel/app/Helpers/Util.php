...
class util
{
    ...
    public static function getBatteryPercentage($batteryVoltage){

        $batteryVoltage = floatval($batteryVoltage);

        if($batteryVoltage >= 12.73){

            return "100%";

        }else if($batteryVoltage >= 12.62){

            return "90%";

        }else if($batteryVoltage >= 12.50){

            return "80%";

        }else if($batteryVoltage >= 12.37){

            return "70%";

        }else if($batteryVoltage >= 12.24){

            return "60%";

        }else if($batteryVoltage >= 12.10){

            return "50%";

        }else if($batteryVoltage >= 11.96){

            return "40%";

        }else if($batteryVoltage >= 11.81){

            return "30%";

        }else if($batteryVoltage >= 11.66){

            return "20%";

        }else if($batteryVoltage >= 11.51){
    ...
    public static function computeDailyConsumption($date){

        $dataBDD = new \Models\PVData();

        $initialDateBASE = $dataBDD->getDataByTypeAndDate("teleinfoBASE", $date, "DESC");
        $initialDateBASE = $initialDateBASE[0]["value"];

        if(isset($initialDateBASE)){

            $beforeDateBASE = $dataBDD->getDataByTypeAndDate("teleinfoBASE", date('Y-m-d', strtotime ('-1 day' , strtotime ($date))), "DESC");
            $beforeDateBASE = $beforeDateBASE[0]["value"];

            $dailyPower = $initialDateBASE - $beforeDateBASE;
    ...
    public static function sendSMSAlert($message){

        $lien = 'https://smsapi.free-mobile.fr/sendmsg?user=USERKEY&pass=PASSWORD&msg='.urlencode($message).'';
        ...
        curl_setopt($curl, CURLOPT_USERAGENT, "PV@STI SMS Alert System - Projet Étudiant");
